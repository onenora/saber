...
if (url.includes('/note/imagefeed?') || url.includes('/note/feed?')) {
    ...
    if (obj?.data?.length > 0) {
        if (obj.data[0]?.note_list?.length > 0) {
            ...
            // 画质增强
            obj.data[0].note_list[0].images_list = imageEnhance(
                JSON.stringify(images_list),
            );
            // 保存无水印信息
            $.setdata(
                JSON.stringify(images_list),
                'fmz200.xiaohongshu.feed.rsp',
            );
    ...
    if (obj.data?.length > 0) {
        for (let item of obj.data) {
            ...
            if (item?.id && selectedStream?.master_url) {
                ...
                console.log(
                    `提取成功 ➜ ${item.id} → ${selectedStream.stream_desc}`,
                );
                videoData.push(data);
            } else {
...
if (
                    ...
                    if (sub_comment.pictures?.length > 0) {
                        ...
                        for (const picture of sub_comment.pictures) {
                            if (picture.video_id) {
                                ...
                                if (picObj.stream?.h265?.[0]?.master_url) {
                                    console.log(
                                        'video_id1：' + picture.video_id,
                                    );
                                    const videoData = {
                                        videId: picture.video_id,
                                        videoUrl:
                                            picObj.stream.h265[0].master_url,
                                    };
                                    livePhotos.push(videoData);
                                }
                            }
    ...
    if (livePhotos.length > 0) {
        ...
        $.setdata(
            ...
            'fmz200.xiaohongshu.comments.rsp',
        );
    }
}
...
function replaceUrlContent(collectionA, collectionB) {
    ...
    collectionA.forEach((itemA) => {
        const itemB = collectionB.find(
            (itemB) => itemB.file_id === itemA.file_id,
        );
        if (itemB) {
            itemA.url =
                itemA.url !== ''
                        ...
                        .then(
                            ...
                            (t) => {
                                ...
                                e(
                                    i,
                                    r,
                                    r && s.decode(r.rawBody, this.encoding),
                                );
                            },
                        ));
            }
