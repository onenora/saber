...
if (url.includes('/main/init')) {
    ...
    obj.data.forEach((i) => {
      ...
      if (extra && typeof extra === 'object' && !Array.isArray(extra))
        Object.keys(extra).forEach((k) => {
          if (k.startsWith('SplashAd.') || k.startsWith('Ad.')) delete extra[k];
        });
    });
  ...
  if (d) {
    ...
    ['detailSponsorCard', 'include_goods', 'include_goods_ids'].forEach((k) => {
      if (d[k]) d[k] = [];
    });
  }
} else if (url.includes('/feed/replyList')) {
  obj.data = Array.isArray(obj.data) ? obj.data.filter((i) => i?.id) : obj.data;
}
