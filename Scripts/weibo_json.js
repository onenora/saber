...
function getModifyMethod(e) {
  for (let t of modifyCardsUrls)
    if (e.indexOf(t) > -1) return "removeCards";
  for (let o of modifyStatusesUrls)
    if (e.indexOf(o) > -1) return "removeTimeLine";
  for (let [i, a] of Object.entries(otherUrls))
    if (e.indexOf(i) > -1) return a;
  return null
}
  ...
  for (let i of t) {
    ...
    if (i.mblog) {
      ...
      if ("bottom_mix_activity" == i.itemid) a = !1;
        ...
        let n = i.card_group;
        if (!n) continue;
        if (["guess_like_title", "cats_top_title", "chaohua_home_readpost_samecity_title"].indexOf(n[0].itemid) > -1) a = !1;
        else if (n.length > 1) {
          let d = [];
...
function removeSearch(e) {
  ...
  let t = [];
  for (let o of e.items)
    if ("feed" == o.category) isAd(o.data) || (o.data?.page_info?.video_limit && delete o.data.page_info.video_limit, t.push(o));
    else {
...
function removeCards(e) {
  ...
  for (let o of e.cards) {
    e.cardlistInfo?.containerid == "232082type=1" && (17 == o.card_type || 58 == o.card_type || 11 == o.card_type) && (o = {
      card_type: o.card_type + 1
    });
    let i = o.card_group;
    if (i && i.length > 0) {
      let a = [];
...
function itemExtendHandler(e) {
  ...
  mainConfig.removeFollow && e.follow_data && (e.follow_data = null), mainConfig.removeRewardItem && e.reward_info && (e.reward_info = null), e.page_alerts && (e.page_alerts = null);
  try {
...
function removeHome(e) {
  ...
  for (let o of e.items) {
    let i = o.itemId;
    if ("profileme_mine" == i) mainConfig.removeHomeVip && (o = removeHomeVip(o)), o.header?.vipIcon && delete o.header.vipIcon, updateFollowOrder(o), t.push(o);
    else if ("100505_-_top8" == i) updateProfileSkin(o, "profileSkin1"), t.push(o);
    else if ("100505_-_newcreator" == i) "grid" == o.type ? (updateProfileSkin(o, "profileSkin2"), t.push(o)) : mainConfig.removeHomeCreatorTask || t.push(o);
    else {
...
function removeComments(e) {
  ...
  let i = [];
  for (let a of o) {
    let r = a.adType || ""; - 1 == t.indexOf(r) && 6 != a.type && i.push(a)
  }
